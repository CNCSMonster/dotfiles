...
function install-helix-lsp() {
    ...
    if command -v node >/dev/null 2>&1; then
        if ! command -v typescript-language-server >/dev/null 2>&1; then
            ...
            install-typescript-lsp && { ((++installed)); true; } || { ((failed++)); true; }
        else
    ...
    if command -v zig >/dev/null 2>&1; then
        if ! command -v zls >/dev/null 2>&1; then
            echo "[安装] zls (依赖：zig)"
            install-zls && { ((++installed)); true; } || { ((failed++)); true; }
        else
            echo "[跳过] zls 已安装"
            ((skipped++))
        fi
    ...
    if command -v pnpm >/dev/null 2>&1 || command -v npm >/dev/null 2>&1; then
        if ! command -v yaml-language-server >/dev/null 2>&1; then
            ...
            install-yaml-lsp && { ((++installed)); true; } || { ((failed++)); true; }
        else
    ...
    if ! command -v taplo >/dev/null 2>&1; then
        ...
        install-taplo && { ((++installed)); true; } || { ((failed++)); true; }
    else
        echo "[跳过] taplo 已安装"
        ((skipped++))
    fi
    ...
    if ! command -v lua-language-server >/dev/null 2>&1; then
        ...
        install-lua-lsp && { ((++installed)); true; } || { ((failed++)); true; }
    else
    ...
    if command -v node >/dev/null 2>&1; then
        if ! command -v bash-language-server >/dev/null 2>&1; then
            ...
            install-bash-lsp && { ((++installed)); true; } || { ((failed++)); true; }
        else
...
function install-sccache() {
    # sccache 安装到 ~/.cargo/bin/sccache（已在 PATH 中）
    if command -v sccache &>/dev/null; then
        echo "sccache 已安装，跳过"
        return 0
    fi
